...
| JK13 | Die Option **je Fremdjoker einzeln** einstellbar, nicht einmal für alle | ✅ | Drei Wege, alle mit demselben Muster (Standard oben, Abweichung je Art): die **Grundform** (Widerruf, Sichtbarkeit, Abklingzeit, Verfall, Bedingung) über `JOKER_ARTEN`, die **Sperrfrist** über `eingriffe.sperrfrist` samt Vertiefung, und die **Stärke** ohnehin je Art (`klau.anteil`, `block.restanteil`, `trittbrett.anteil`, `gegenwette.einsatz`). ⏳ Bewusst gemeinsam bleibt allein `sichtbarVorFrist` — Andi hat am 23.08.2026 nur die Sperrfrist einzeln bestellt |
