...
| PP1b | **Die Gesamtspiel-Bibliothek als VORAUSWAHL ganz am Anfang** — „gleich am anfang mit dem code auch die Gesamtspielbibliothek mit den Complete game einstellungen als vorauswahl und oben auch filter mit relevanz etc und suchenfunktion" (Andi, 24.08.2026, **nach eigener Angabe zum dritten Mal**) | ✅ | 🔴 **Der Fehler war, PP1 für erledigt zu halten.** Das FENSTER hatte alles — Suche, Filter, Relevanz —, stand aber hinter einem Chip. Am Anfang des Screens lag `RundenCharaktere`: vier Karten aus `CHARAKTERE`, **ohne Suche, ohne Filter, ohne Sortierung und ohne die Regelwerke**. Eine Vorauswahl beantwortet die erste Frage des Screens; ein Nachschlagewerk hinter einem Chip beantwortet sie nur für die, die von ihm wissen. Neu: `GesamtspielAuswahl.jsx` an derselben Stelle, mit Suchfeld, Art-Filter (Alle · 🎯 Runden-Ideen · 📐 Regelwerke) und Sortierung (Relevanz · Verbreitung · Name), dazu „Alle anzeigen" ins Fenster für die Bausteine. **„Complete Game" = `charakter` + `preset`**, Bausteine bleiben draußen: sie mischen sich in ein Regelwerk, sie ersetzen keins. `RundenCharaktere.jsx` gelöscht. Gemessen auf 390 px: **11 Einträge statt 4**, Suche „joker" → 4 von 11, keine Seitenfehler. Übernahme läuft über EIN `uebernimmEintrag` für Fenster und Vorauswahl |
