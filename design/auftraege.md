...
| SA1 | **K.-o.-Phasen je Wettbewerb als LISTE einstellbar** — „bei den verschiedenen Liga-Cups, dass ab Viertelfinale auch betippt wird" (Andi, 24.08.2026) | 🔨 | **Die Mechanik steht schon:** `spiele.phasen` filtert nach Phase (`spielauswahl.js`), `jeWettbewerb` erlaubt Abweichungen je Liga, und `npm run greift` misst den Fall „nur CL ab Achtelfinale" bereits mit **15 Spielen**. ⏳ Offen ist die **Liste in der Oberfläche**: je Wettbewerb eine Zeile, in der die Phase ab-wann gewählt wird |
| SA2 | 🔴 **Bedingung „nur wenn Mannschaft X beteiligt ist"** — „mit Option dass das nur gilt, sobald die und die Mannschaft beteiligt ist, also bspw. Barca oder Real" (Andi, 24.08.2026) | ⏳ | **Das gibt es noch NICHT, und es bricht mit einer bestehenden Entscheidung** — deshalb hier ausdrücklich vermerkt: `spielauswahl.js` verknüpft alle Dimensionen **UND** (`VERKNUEPFUNG_HINWEIS`: „Vereine, Zeitraum, Wettbewerbe, Phasen müssen gemeinsam zutreffen"), und eine ODER-Verknüpfung wurde dort bewusst verworfen („wäre eine zweite, konkurrierende Regel-Sprache"). Andis Wunsch ist genau so ein Fall: *ab Viertelfinale* **ODER** *wenn Barça spielt*. ⚠️ **Vor dem Bauen zu klären:** ob das als dritte Dimension IM Wettbewerbs-Eintrag gelöst wird (`jeWettbewerb.cl = { abPhase: "viertelfinale", nurWenn: ["barca","real"] }`) — das bliebe innerhalb einer Zeile und damit UND-verknüpft — oder ob wirklich eine ODER-Sprache nötig ist. Die erste Variante halte ich für richtig |
| SA3 | **Variable Vorschau beim Erstellen: wie viele Spiele über die Saison und pro Woche** (Andi, 24.08.2026) | 🔨 | **Halb gebaut.** `aufwand.js` + `AufwandPanel.jsx` rechnen schon **je Spieltag** und nennen ausdrücklich den **Median** statt des Mittelwerts (ein englischer Doppelspieltag zöge den Schnitt hoch und meldete den Normalfall als Problem). ⏳ Was fehlt: die **Gesamtzahl über die Saison** daneben, und dass sich beides **live mitbewegt**, während man die Auswahl ändert. ⚠️ Die Zahl gibt es bereits an anderer Stelle — die Startseite zeigt „1943 Spiele · 1854 offen"; sie gehört in die Erstellung, wo die Entscheidung fällt |
