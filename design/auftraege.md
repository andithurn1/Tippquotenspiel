...
| TI9 | **Modell für Ergebnisse ohne Originalquote** — „ausgehend von der naechsten Quote" (Andi, 22.08.2026) | 🔨 | Befund gemessen und Vorschlag steht in `design/randquoten.md`: 1,65 % der Spiele enden ausserhalb des 6x6-Rasters, ein exakt getipptes 6:0 zahlt **47** statt **1440** Punkte. Vorschlag: (a) Raster groesser erzeugen (`rasterAusMarkt` nimmt `grid` schon als Parameter), (b) Randfortschreibung als Sicherung, markiert wie `spielerQuelle`. ⚠️ Gehoert in die QUOTE (Ebene 1), nicht in den Modifikator-Topf — dort frisst es `modCap`. Zwei ❓ offen: Rastergroesse und ob die Fortschreibung auch bei echten Marktquoten greift |
