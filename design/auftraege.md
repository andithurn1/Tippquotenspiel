...
| ST5 | **Kopfzeile: Bibliothek · Gamemode · GameCode einsetzen** | ✅ | Drei Chips in der klebenden Kopfzeile (`KopfChip` in `Spielerstellung.jsx`), jeder zeigt seinen STAND und springt zu seinem Abschnitt. Die Werte sind abgelesen statt gemerkt — kein zweiter Zustand neben `presetKey`/`rules`/`shortCode`. Im Browser geprüft: aus Position 1800 landet das Ziel exakt unter der Kopfzeile, „Wettbüro“ wählen ändert 📚 und 🎮 sofort mit. Auf 375 px alle drei 44 px hoch, kein Querlauf. ⚠️ `behavior: "smooth"` musste raus — damit passierte gar nichts |
...
| JK5 | **Cooldown je Ziel**, damit nicht immer dieselben getroffen werden | ✅ | `eingriffe.sperrfristJeZiel` (0–20, Vorgabe 0 = aus), geprüft in `zulaessigeZiele`. 🔴 **Dabei ein Fund:** `maxProZiel` und `immun` zählten bis dahin nur die EIGENEN Einsätze — obwohl ihre Karte „Schutz der Getroffenen“ heißt und verspricht, dass sich nicht die ganze RUNDE auf eine Person einschießt. Fünf Spieler durften denselben fünfmal treffen, ohne dass eine Schranke ansprach. Jetzt sagen alle drei genau das, was auf ihnen steht: `maxProZiel`/`immun` ziel-bezogen, `sperrfristJeZiel` paar-bezogen |
| JK6 | 🔴 Eingriffe müssen **vor der Frist sichtbar und zurücknehmbar** sein | ✅ | `eingriffe.sichtbarVorFrist` + `eingriffe.ruecknahme` (bisAnpfiff · bisFrist · nein), ausgewertet in `eingriffFenster()` über den `frist`-Zustand des Tippfensters. **Und die Spieler-Ansicht dazu**, ohne die der Zweck wegfiele: `MeineJoker` zeigt „Lena · Block · Bayern – Dortmund · noch bis zum Anpfiff“ samt NAMEN — die Daten kommen aus der neuen Store-Methode `getFremdEingriffe`, nicht aus einer zweiten Rechnung im Screen. `ruecknahme: "nein"` meldet `konflikte()` als Widerspruch zum Zweck der Familie |
...
| JK12 | **Ausgeloste Zielperson** statt freier Opferwahl — man sucht sich das Ziel nicht aus, entscheidet aber bei der Tippabgabe, bei welchem EINZELSPIEL man einsetzt | 🔨 | Vorlage in `joker-sondermenue.md` Teil D. **Alle fünf Fragen am 23.08.2026 entschieden**: Andi — wo es mehrere sinnvolle Varianten gibt, wird die Variante zur EINSTELLUNG. Also `losTakt` (Spieltag · Saison · nach Einsatz), `losPaare`, `losSichtbar`, je Fremdjoker einzeln. Der Fall „Los tippt nicht" wird nicht behandelt, sondern über den Auto-Tipp VERHINDERT. Bauen erst nach JK18 |
| JK13 | Die Option **je Fremdjoker einzeln** einstellbar, nicht einmal für alle | 🔨 | Halb da seit 23.08.2026: die vier Arten stehen jetzt in `JOKER_ARTEN` (`eingriffe.trittbrett`, `eingriffe.gegenwette`), also greift die **Grundform** je Art — Widerruf, Sichtbarkeit, Abklingzeit, Verfall, Bedingung sind einzeln stellbar, mit dem vorhandenen Muster (Standard oben, Abweichung je Art), kein zweites erfunden. ⏳ Offen bleiben die drei FAMILIEN-Werte (`sperrfristJeZiel`, `sichtbarVorFrist`, `ruecknahme`): sie gelten heute für alle vier gemeinsam. ❓ Ob sie das sollen — eine Sperrfrist, die für den Block anders ist als für die Gegenwette, ist denkbar, aber niemand hat sie bestellt |
