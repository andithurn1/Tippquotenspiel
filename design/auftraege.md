...
| JK18 | 🔴 **Zwei-Phasen-Spieltag als Voraussetzung der Fremdjoker** — erst tippen alle, einen Tag später werden die Joker auf die anderen gesetzt | ⏳ | **Andis Ansage vom 23.08.2026**, aus der Antwort auf JK12 Frage 4. Das heutige Tipp-Fenster kennt zwei Kanten (öffnet `vorlaufStunden` vor Anpfiff, schließt BEIM Anpfiff); für Phase 2 braucht es eine dritte: einen **Tippschluss vor dem ersten Anpfiff**. Hängt zwingend an `tippfenster.anker: "spieltag"` — bei `"spiel"` gibt es keinen gemeinsamen Moment, an dem „jeder getippt hat". Pflichtangabe, sobald Fremdjoker an sind; ohne sie darf sich die Familie NICHT einschalten lassen. Vollständig in `joker-sondermenue.md`, Abschnitt „Zwei-Phasen-Spieltag" |
| JK19 | Beim Einschalten der Fremdjoker der **ehrliche Hinweis**: eure Runde muss zweimal pro Spieltag reinschauen | ⏳ | Andi, 23.08.2026: „da muss eigentlich jeder genug Hingabe für aufbringen bzw. regelmäßig genug reinschauen bei dieser Option." Gehört in die Oberfläche, nicht nur ins Dokument — und in seiner Sprache, nicht als Systemmeldung. Gegenstück zu JK7 (Büro-Runde schaltet die Familie ganz aus) |
