  ...
  if (!abweichend) {
    return (
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 12.5, fontWeight: 700 }}>{art.label}</div>
...
function BasisFelder({ basisRoh, effektiv, onPatch, onPatchBedingung }) {
  ...
  return (
    <div>
      ...
      <Feldgruppe titel="Kasse sichtbar — sehen andere Münzstand und Bestand?">
        <ToggleKnopf an={wert("kasseSichtbar")} onChange={(v) => onPatch({ kasseSichtbar: v })}
          textAn="Offen — jeder sieht Münzen und Bestand" textAus="Verdeckt — jeder Einsatz bleibt eine Überraschung" />
      </Feldgruppe>
      ...
      <Feldgruppe titel="Umfang — worauf ein Einsatz wirkt">
        ...
        {umfangAktuell !== "spieltag" && (
          <div style={{ marginTop: 8 }}>
            <div style={{ fontSize: 10.5, color: C.muted, marginBottom: 5 }}>Welche Spiele</div>
      ...
      <Feldgruppe titel="Bedingung — worauf der Joker überhaupt gilt">
        ...
        <div style={{ marginTop: 8 }}>
          <div style={{ fontSize: 10.5, color: C.muted, marginBottom: 5 }}>Phasen (leer = alle)</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {PHASEN.map((p) => (
