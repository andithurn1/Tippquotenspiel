...
export default function Konto() {
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: "100%", maxWidth: "var(--tqs-schirm-breite)", borderRadius: RUND.schirm, overflow: "hidden",
        ...
        <div style={{ marginTop: 6, fontSize: "1.25rem", fontWeight: 700 }}>Konto & Daten</div>

        {isMock ? (
          <p style={{ fontSize: "0.8125rem", color: C.muted, marginTop: 10, lineHeight: 1.5 }}>
        ...
        <Divider />
        <div style={{ fontSize: "0.75rem", color: C.muted, lineHeight: 1.7 }}>
...
function NameCard({ user, updateName }) {
  ...
  return (
    <form onSubmit={save} style={{ marginTop: 18 }}>
      <div style={{ fontSize: "0.9375rem", fontWeight: 700 }}>Anzeigename</div>
      ...
      {state === "saved" && <div style={{ fontSize: "0.75rem", color: C.mint, marginTop: 6 }}>✓ Gespeichert.</div>}
...
function ExportCard({ exportMyData }) {
  ...
  return (
    <div>
      <div style={{ fontSize: "0.9375rem", fontWeight: 700 }}>Meine Daten exportieren</div>
      <div style={{ fontSize: "0.75rem", color: C.muted, marginTop: 4, lineHeight: 1.5 }}>
      ...
      <button onClick={run} disabled={state === "working"} style={{
        ...
        background: C.surface2, color: C.text, fontWeight: 700, fontSize: "0.8125rem",
...
function DeleteCard({ email, deleteAccount }) {
  ...
  return (
    <div>
      <div style={{ fontSize: "0.9375rem", fontWeight: 700, color: C.coral }}>Konto löschen</div>
      <div style={{ fontSize: "0.75rem", color: C.muted, marginTop: 4, lineHeight: 1.5 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
        ...
        <button onClick={run} disabled={!armed || state === "working"} style={{
          ...
          fontWeight: 700, fontSize: "0.9375rem", border: `1px solid ${armed ? C.coral : C.line}`,
          borderRadius: RUND.karte, padding: "0 16px", fontFamily: "inherit",
        }}>{state === "working" ? "…" : "Löschen"}</button>
      </div>
      {state === "error" && <div style={{ fontSize: "0.75rem", color: C.coral, marginTop: 6 }}>{err}</div>}
      <div style={{ fontSize: "0.6875rem", color: C.muted, marginTop: 10, lineHeight: 1.6 }}>
